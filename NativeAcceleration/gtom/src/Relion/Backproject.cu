...
namespace gtom
{
	...
	void d_rlnBackproject(tcomplex* d_volumeft, tfloat* d_volumeweights, int3 dimsvolume, tcomplex* d_projft, tfloat* d_projweights, int3 dimsproj, uint rmax, glm::mat3* d_matrices, int* d_ivolume, float4 magnification, float ewaldradiussuper, bool outputdecentered, bool squareinterpweights, uint batch)
	{
		...
		if (ndimsvolume == 3)
		{
			...
			dim3 grid = dim3(tmin(256, (elements + 128 - 1) / 128), batch, 1);
																											...
																											int rmax2)
	{
		...
		for (uint id = blockIdx.x * blockDim.x + threadIdx.x; id < elementsproj; id += blockDim.x * gridDim.x)
		{
			uint idx = id % dimft;
			...
			for (uint i = 0; i < 8; i++)
			{
				...
				position.y = tmax(0, tmin((int)dimvolume - 1, position.y));
				position.z = tmax(0, tmin((int)dimvolume - 1, position.z));
