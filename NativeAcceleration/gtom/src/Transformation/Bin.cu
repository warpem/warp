...
namespace gtom
{
	...
	__global__ void Bin2DKernel(tfloat* d_output, int width, cudaTex inputTex);
	...
	void d_Bin(tfloat* d_input, tfloat* d_output, int3 dims, int bincount, int batch)
	{
		for (int b = 0; b < batch; b++)
		{
			...
			else if (dims.z <= 1)			//2D
			{
				...
				{
					// Create CUDA array and copy data
					cudaChannelFormatDesc desc = cudaCreateChannelDesc<tfloat>();
					cudaArray_t a_input;
					cudaMallocArray(&a_input, &desc, dims.x / (1 << i), dims.y / (1 << i));
					
					// Copy data to array
					d_CopyToArray(i == 0 ? d_input + elements * b : d_intermediate, 
						a_input, 
						dims.x / (1 << i), 
						dims.y / (1 << i), 
						1, 
					...
					cudaTex inputTex = d_CreateTextureObject(a_input, 
					...
					Bin2DKernel << <grid, (uint)TpB >> > (d_result, dims.x / (2 << i), inputTex);
	...
	__global__ void Bin2DKernel(tfloat* d_output, int width, cudaTex inputTex)
	{
		for (int x = blockIdx.x * blockDim.x + threadIdx.x;
			...
			d_output[blockIdx.y * width + x] = tex2D<tfloat>(inputTex, (float)(x * 2 + 1), (float)(blockIdx.y * 2 + 1));
	}
