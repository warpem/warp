...
public class WorkerControllerHost : IDisposable
{
    ...
    {
        _controllerService = new WorkerControllerService(WorkDistribution.Instance);
    }
    
    public WorkerControllerHost(WorkerControllerService sharedService)
    {
        _controllerService = sharedService ?? throw new ArgumentNullException(nameof(sharedService));
    }
